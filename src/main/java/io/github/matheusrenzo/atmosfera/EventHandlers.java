...
public class EventHandlers implements Listener {
    ...
    public EventHandlers(Atmosfera plugin) {
    ...
    public void onCommandPreprocess(PlayerCommandPreprocessEvent event) {
        if ((config.getBlockTimeSetCommand() && config.isTimeEnabled() && event.getMessage().contains("time set"))
                || (config.getBlockWeatherCommand() && config.isWeatherEnabled()
                        && event.getMessage().contains("weather"))) {
            event.setCancelled(true);
            event.getPlayer().sendMessage("Comando desativado pelo Atmosfera");
        }
    }
    ...
    public void onConsoleCommand(ServerCommandEvent event) {
        if ((config.getBlockTimeSetCommand() && config.isTimeEnabled() && event.getCommand().contains("time set"))
                || (config.getBlockWeatherCommand() && config.isWeatherEnabled()
                        && event.getCommand().contains("weather"))) {
            event.setCancelled(true);
            event.getSender().sendMessage("Comando desativado pelo Atmosfera");
        }
    }
    ...
    public void onPlayerBedEnter(PlayerBedEnterEvent event) {
        ...
        if (config.isTimeEnabled() && config.getDisableBedsAtNight()
                && ((!playerWorld.hasStorm() && worldTime >= 12542 && worldTime <= 23459)
